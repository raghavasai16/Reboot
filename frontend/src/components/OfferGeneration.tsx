      ...
      <Card className="p-6">
        ...
        {/* <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <div className="text-xs text-yellow-700 space-y-1">
            <div>HR Review Data: {JSON.stringify(hrReviewData, null, 2)}</div>
            <div>Window Selected Candidate: {JSON.stringify((window as any).selectedCandidate, null, 2)}</div>
            <div>Current Offer Data: {JSON.stringify(offerData, null, 2)}</div>
            <div>Candidate Prop: {JSON.stringify(candidate, null, 2)}</div>
          </div>
          <div className="mt-3">
            <button 
              onClick={() => {
                console.log('Manual test - HR Review Data:', hrReviewData);
                console.log('Manual test - Offer Data:', offerData);
                console.log('Manual test - Candidate:', candidate);
                if (hrReviewData) {
                  setOfferData(prev => ({
                    ...prev,
                    candidateName: hrReviewData.candidateName || 'Test Candidate',
                    position: hrReviewData.candidatePosition || 'Test Position',
                    department: hrReviewData.candidateDepartment || 'Test Department',
                    fixedCTC: hrReviewData.fixedCTC ? hrReviewData.fixedCTC.toString() : '1000000',
                    joiningDate: hrReviewData.joiningDate || '2024-12-01'
                  }));
                }
              }}
